...
playSound _sound;
{
	switch (_x) do 
	{
		...
		case "epochMsg": {
			if (GMSCore_modType isEqualTo "epoch") then {[_msg,5] call Epoch_message};
		};
		case "toast": {
			if (GMSCore_modType isEqualTo "exile") then {["InfoTitleAndText", [_label, _msg]] call ExileClient_gui_toaster_addTemplateToast};
		};
		case "dynamic": {
			_text = format[
