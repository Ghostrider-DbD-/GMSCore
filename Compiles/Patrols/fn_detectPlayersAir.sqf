...
private _maxEnemyDist = _group getVariable["maxEnemyDist", DISTANCE_NEAREST_ENEMY_AIR];
...
if (canMove _veh && _aliveUnits > 0) then {
    _detected = (getPosATL _veh) nearEntities[[GMSCore_playerUnitTypes append ["LandVehicle"]], PLAYER_DETECT_RANGE_AIR];
    if (count _detected > 5) then {_detected resize 5};
    if (count _detected > 0) then {      
        [_group, _detected] call GMSCore_fnc_setDetectedTargets;      
        _target = _detected select 0; 
        private _distance = _veh distance2D _target;
        private _targetPos = getPosATL _target; 
        private _waypointDisallowedAreas = nearestLocations[_targetPos,[WAYPOINT_DISALLOWED_AREAS], WAYPOINT_DISALLOWED_DISTANCE_AIR];
        ...
        private _isBlacklisted = [_targetPos, _waypointDisallowedAreas, "GMSCore_fnc_detectPlayerAir"] call GMSCOre_fnc_isBlacklisted;
        ...
        if !(_isBlackListed) then {
            ...
            {
                if (!([_targetPos] call GMSCOre_fnc_inAllowedLocation) && (_targetPos distance2D _veh > _maxAgroDist)) then {
                    ...
                    if (_canReveal && (_group knowsAbout _x < 2) && (random(1) < _chanceDetected)) then {
                        ...
                        // set the target as the closest one in the list of targets                        
        ...
        if (_numberTargets > 0) then {
            ...
            [_group, []] call GMSCore_fnc_setDetectedTargets;
        };
    };
};
