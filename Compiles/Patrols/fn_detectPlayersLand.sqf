...
if (canMove _veh && _aliveUnits > 0) then {
    ...
    if (count _detected > 5) then {_detected resize 5};
        ...
        [format["_detectPlayersLand: __target %1 | _targetPos %2 | _waypointDisallowedAreas %3",_target, _targetPos, _waypointDisallowedAreas]] call GMSCore_fnc_log;
        private _isBlacklisted = [_targetPos, _waypointDisallowedAreas, "GMSCore_fnc_detectPlayersLand"] call GMSCOre_fnc_isBlacklisted;
