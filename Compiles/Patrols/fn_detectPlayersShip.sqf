...
//private _blacklistedAreas = _group getVariable [GMSCore_blackListedAreas, []];
...
if (canMove _veh && _aliveUnits > 0) then {
    ...
    if (count _detected > 0) then {            
        ...
        private _isBlacklisted = [_targetPos, _waypointDisallowedAreas] call GMSCOre_fnc_isBlacklisted;
        ...
        if (_numberTargets > 0) then {
            if (_huntMrkr isEqualTo "") then {
                private _huntMrkr = createMarkerLocal[format["huntM%1",_group]];
            };
            private _markerPos = getPosATL (_detected select 0);
            _huntMarker setMarkerPosLocal _markerPos;
            _huntMrkr setMarkerShapeLocal "ELIPSE";
            _huntMrkr setMarkerSizeLocal [300, 300]; 
            _group setVariable["huntMrkr", _huntMrkr];
            [_group, true] call GMSCore_fnc_setHunt;
            [leader _group, "Hunt"], call GMSCore_fnc_nextWaypointAreaPatrolAir;
        };
    };
};
