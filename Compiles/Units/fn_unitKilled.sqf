...
//[format["\x\addons\GMSCore_fnc_unitKilled: _unit = %1 | _killer = %2 | _instigator %3",_unit,_killer,_instigator]] call GMSCore_fnc_log;
//[format["\x\addons\GMSCore_fnc_unitKilled: count (group _unit) getVariable[GMS_aiKilledCode,[]] = %1",count ((group _unit) getVariable[GMS_aiKilledCode,[]])]] call GMSCore_fnc_log;
...
if !(isNull _veh) then 
{
	...
	//[format["\x\addons\GMSCore_fnc_unitKilled: vehicle crew = %1 || alive vehicle crew = %2",crew _veh, _alive]] call GMSCore_fnc_log;
	if (_alive == 0) then 
	{
		private _accessAllowed = _veh getVariable [GMS_allowAccess,true];
		//[format["\x\addons\GMSCore_fnc_unitKilled: _veh = %1 | _accessAllowed = %2",_veh,_accessAllowed]] call GMSCore_fnc_log;
