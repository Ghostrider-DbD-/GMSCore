...
try {
	if !(isNull _target && !_stuck) throw 2;
	if (_stuck) throw 3; 
	//  Handle a normal waypoint update 
	throw 1;
}
	...
	switch (_exception) do {
		...
		case 2: {  //  Hunt a player 
			private "_vector";
			private _distance = (selectRandom [175, 200, 225]) max (((leader _group) distance _target)) * 0.8;
			if ((leader _group) distance _target < 250) then {
				_vector = (_leader getRelDir (position _target)) + (random(45) * selectRandom[1, -1]);
			} else {
				_vector = (_leader getRelDir (position _target)) + (random(30) * selectRandom[1, -1]);
			};
			private _nextPos = (position _target) getPos [_vector, _distance]; 
			...
			(leader _group) commandSuppressiveFire _target;
			_group reveal[_target, _group getVariable[GMS_patrolIntelligence, 0.25]];
			_wp setWaypointPosition [_nextPos,0];
