...
if (isNil "_markerSize") then {
	_markerSize = [_mapRange / 2, _mapRange / 2];
};
if (isNil "_markerShape") then {
	_markerShape = "RECTANGLE";
};

//[format["_configureWorld: _markerSize %1 | _markerPosn %2 | _mapRange %3",_markerSize, _markerPosn, _mapRange]] call GMSCore_fnc_log;
