...
addMissionEventHandler ["OnUserSelectedPlayer", {
		...
		_playerObject triggerDynamicSimulation true;
		[format["Setting triggerDynamicSimulation true for playerObject %1 | _name %2", _playerObject, name _playerObject]] call GMSCore_fnc_log;
	};
];
...
[] spawn {
	...
	if ((toLowerANSI GMSCore_modType) isEqualTo "exile") then
	{
		...
		while {isNil "PublicServerIsLoaded"} do {uiSleep 1};
		while {isNil "ExileServerIsLocked"} do {uiSleep 1};
